...
/**
 * Handles the ending of a statement:
 * wow [text]
 */
function handleWow(keys)
{
  expectToken('wow', keys);
  multiLine = false;
...
/**
 * Handles the ending of a chained statement:
 * wow& [text]
 */
function handleWowAnd(keys)
{
  expectToken('wow&', keys);
  multiLine = false;
...
module.exports = function parse (line) {
   ...
   // such function
    ...
    if ( keys[0] === 'wow' )
    {
      statement += handleWow(keys);
    }

    if ( keys[0] === 'wow&')
    {
      statement += handleWowAnd(keys);
    }
