...
/**
 * Raises an error if the given tokens do not start with either of the desired states
 */
function expectEitherToken(expectedStart, alternateStart, tokens)
{
  if ( tokens[0] !== expectedStart && tokens[0] !== alternateStart)
  {
    throw new Error(`Invalid parse state! Expected: '${expectedStart}' or '${alternateStart}' but got: '${tokens[0]}' from chain: [${tokens}]`);
  }
}
 ...
 * wow& [return vals]
 */
function handleWow_WowAnd(keys)
{
  expectEitherToken('wow', 'wow&', keys);
  multiLine = false;
...
module.exports = function parse (line) {
   ...
   // such function
    ...
    {
      statement += handleWow_WowAnd(keys);
    }
    ...
    if (keys[0] === 'but') {
        if (keys[1] === 'rly') {
          statement += '} else if (';
          statement += controlFlowParser(2, keys);
          statement += ') {\n';
