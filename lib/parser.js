...
// can probably be applied as a binary operator since it's a simple transform
...
function isDogescriptSource(parseContext)
{
  ...
  // handling a comparison in any spot
  if(tokens.some(isComparisonOperator))
  {
    return true;
  }
...
/**
 * Determines whether the next token in the chain can be separated from the current chain with a ' ' or not
 */
function shouldSeparateTokens(tokens)
{
  var nextToken = tokens[0];
  // giv needs to be appended next to the previous token to produce x.y and not x . y
  return nextToken !== 'giv';
}

/**
 * Determines whether the token is any of the supported comparison operators
 */
function isComparisonOperator(token)
{
  return comparisonOperators.hasOwnProperty(token);
}
...
function parseStatement(parseContext) {
  ...
  if (isComparisonOperator(tokens[0]))
  {
    var statement = comparisonOperators[tokens.shift()];
    if (tokens.length > 0) {
      statement += ' ';
    }
    return statement;
