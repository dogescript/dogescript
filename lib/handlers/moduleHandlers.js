...
function handleWoof(parseContext)
{
  ...
  throw new SyntaxError(`Invalid parse state! Expected a value but got: '${tokens[0]}' from chain: [${tokens}]. Allowed construct 'woof [<name> be] <value | <SUCH> | <MUCH> >'. ${tokenUtils.parseInfo(parseContext)}`);
}
