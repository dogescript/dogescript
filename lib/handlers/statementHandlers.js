...
function handleWow(parseContext) {
  ...
  // turn off mulitline object declaration
  if (
    parserState.hasState(StateEnum.OBJECT) ||
    // mark end function declaration
    parserState.hasState(StateEnum.FUNCTION) ||
    // mark end of class declaration
    parserState.hasState(StateEnum.CLASS) ||
    // mark end of control flow block
    parserState.hasState(StateEnum.CONTROL_FLOW_BLOCK)
  ) {
...
function handleRly(parseContext) {
  ...
  var statement = "if (";
  parserState.pushState(StateEnum.CONTROL_FLOW_BLOCK);

  parserState.pushState(StateEnum.CONTROL_FLOW_CONDITION);
...
function handleVery(parseContext) {
  ...
  statement += handleAssignmentExpression(parseContext);

  return statement;
}

/**
 * Handles an assignment expression , the right side of an <x> = <y>:
 *  is/more/less/lots <expression>
 *
 * Requires the consumer to have consumed the assignment token.
 */
function handleAssignmentExpression(parseContext) {
  var tokens = parseContext.tokens;
  if (operatorHandlers.isAssignmentOperator(tokens[0])) {
    throw new Error(
      `Expected an upstream call to consume operator but got ${
        parseContext.tokens[0]
      }. ${tokenUtils.parseInfo(parseContext)}`
    );
  }
