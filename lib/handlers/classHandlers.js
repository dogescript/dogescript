...
function handleSit(parseContext) {
    ...
    if (tokens.length < 1) {
        throw new SyntaxError(`Expected setter name but got nothing. Allowed construct: sit [name] much [arg]. ${tokenUtils.parseInfo(parseContext)}`);
    }
...
function handleStay(parseContext) {
  ...
  var tokens = parseContext.tokens;
    ...
    if (tokens.length < 1) {
        throw new SyntaxError(`Expected function name but got nothing. Allowed construct: stay [name] <much [arg]>. ${tokenUtils.parseInfo(parseContext)}`);
    }
...
module.exports = {
    handleGit, handleSit , handleClass, handleGrows, handleMaker, handleStay
}