...
function handleNoSync(parseContext) {
  tokenUtils.expectToken("nosync", parseContext);
  ...
  throw new Error(
    `Invalid parse state! Expected: 'such' or 'much' but got: '${
      tokens[0]
    }' from chain: [${tokens}]. Allowed construct 'nosync such <function_name> [much <args>]' or 'nosync much <args>'. ${tokenUtils.parseInfo(
      parseContext
    )}`
  );
}
