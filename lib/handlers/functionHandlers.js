...
function handleSuch(parseContext) {
  ...
  var inClass = parserState.hasState(StateEnum.CLASS);

  parserState.pushState(StateEnum.FUNCTION);
  ...
  var statement =
    (inClass ? "" : "function") + (generator ? "*" : "") + " " + functionName;
 ...
 *   asink [such_expression]
 *   asink [much_expression]
 *
 * Produces:
 *   async function (args) {
 */
