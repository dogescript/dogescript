 ...
 *   instanceof
 */
function handleIsA(parseContext) {
  tokenUtils.expectToken("isa", parseContext);
  var tokens = parseContext.tokens;
...
function handleKindOf(parseContext) {
  ...
  if (tokens.length < 1) {
    throw new SyntaxError(
      `Expected argument but got nothing. Allowed construct: kindof [arg]. ${tokenUtils.parseInfo(
        parseContext
      )}`
    );
  }
